...
import {FlatList, Image, Modal, TouchableNativeFeedback, View} from 'react-native';
import {Button, Caption, Chip, IconButton, ProgressBar, Text, Title} from 'react-native-paper';
...
import { useContinueReadingChaptersList, useSettingsContext } from 'src/prodivers';
...
export default function VolumesList({defaultCoverUrl, onVolumeSelect}: Props) {
  ...
  useEffect(() => {
    updateSetting('volumeSortOrder', sortRule === SortRule.Asc ? 'asc' : 'desc');
  }, [sortRule]);
      ...
      <BasicList
        ...
        ListEmptyComponent={
          <View style={{flex: 1, justifyContent: 'center', alignItems: 'center', marginTop: 30}}>
            ...
            <Caption style={{textAlign: 'center'}}>No volumes were found for your language.</Caption>
          </View>
        }
        renderItem={volumeInfo => {
          const volume = volumeInfo.volume;
          const title = volume === 'null' || volume === null ? 'No volume' : `Volume ${volume}`;
          const cover = covers.find(cover => cover.attributes.volume === volume);
          const reading = continueReadingChapters.filter(chapter => volumeInfo.chapterIds.includes(chapter.id));
          ...
          return (
            <TouchableNativeFeedback
              ...
              <View>
                <Image
                  source={{uri: cover ? coverImage(cover, manga.id) : defaultCoverUrl || 'https://mangadex.org/avatar.png'}}
                  width={itemWidth}
                  style={{width: itemWidth, aspectRatio: 1.25, opacity: 0.4}}
                  resizeMode='cover'
                />
                <View style={{position: 'absolute', bottom: 0, left: 0, right: 0, paddingHorizontal: 5}}>
                  <Text>{title}</Text>
                  <Caption style={{marginTop: -3}}>
                    {volumeInfo.chapterIds.length ? pluralize(volumeInfo.chapterIds.length, 'chapter') : 'N/A'}
                  </Caption>
                </View>
                {reading.length > 0 ? <ProgressBar progress={readingPrgress * chapterProgress} style={{height: 1}} /> : null}
              </View>
            </TouchableNativeFeedback>
          );
        }}
        skeletonItem={<ThumbnailSkeleton width={itemWidth} height={itemWidth / 1.25} />}
        skeletonLength={6}
      />
    </View>
  );
